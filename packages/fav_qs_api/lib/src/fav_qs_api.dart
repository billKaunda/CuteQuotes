...
class FavQsApi {
  ...
  Future<void> updateProfile(
    ...
    final jsonRequestBody = UpdateUserRequestRM(
      user: UserInfoRM(
        username: username,
        email: email,
        password: password,
  ...
  Future<void> requestPasswordResetEmail(String email) async {
    ...
    try {
      ...
      //An unregistered email sent to the API will return a 404.
      // This can be considered as a security breach, so, we prefer
      // handling an unregistered email just like a registered one.
